...
- (id)initWithFrame:(CGRect)frame initialTime:(NSInteger)initalTime
{
...
- (void)drawRect:(CGRect)rect
{
    ...
    [bezierPath addArcWithCenter:CGPointMake(rect.size.width / 2, rect.size.height / 2)
                      ...
                      startAngle:0 + initialAngleFactor
                        endAngle:(_percent * M_PI) / (30.0/self.circleFraction) + initialAngleFactor
                       clockwise:YES];
...
//    [[UIColor colorWithRed:0.03f green:0.64f blue:0.83f alpha:1.00f] setStroke];
    ...
    [minuteCounterLabel setCenter:centerOfScreen];
    ...
    if (_minutesLeft > 0) {
        [minuteCounterLabel setText:[NSString stringWithFormat:@"%ld", (long)_minutesLeft]];
    } else {
        [minuteCounterLabel setText:[NSString stringWithFormat:@"%ld", (long)_secondsLeft]];
    }
    ...
    [minutesLabel setCenter:CGPointMake(centerOfScreen.x, centerOfScreen.y + 40)];
